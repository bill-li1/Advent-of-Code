...
static inline int Manhattan(Point a, Point b) {
...
static int Part(int steps) {
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      if (grid[y][x] == '#')
        continue;
      for (int newY = max(0, y - steps); newY <= min(height, y + steps);
           newY++) {
        if (newY < 0 || newY >= height)
          continue;
        for (int newX = max(0, x - steps); newX <= min(width, x + steps);
             newX++) {
          if ((newX < 0 || newX >= width) || (grid[newY][newX] == '#') ||
              (Manhattan({y, x}, {newY, newX}) > steps) ||
              (toStart[y][x] < 0 || toFinish[newY][newX] < 0))
            continue;
          int cheat = toStart[y][x] + toFinish[newY][newX] +
                      Manhattan({y, x}, {newY, newX});
          if (t - cheat >= 100)
            s++;
        }
      }
...
int main() {
  ...
  cout << "Day 20: Race Condition" << endl
       ...
       << "Part 2   - " << Part(20) << endl
       << "run time - "
       << duration_cast<microseconds>(high_resolution_clock::now() - strt)
                  .count() /
              1000.0
       << " ms." << endl;
}

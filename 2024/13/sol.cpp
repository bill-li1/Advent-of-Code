...
static int64_t Solve(bool gt100) {
  ...
  for (auto [a, b, prize] : claw) {
    int64_t rA = 0, rB = 0, x1 = a.x, x2 = a.y, y1 = b.x, y2 = b.y,
            p1 = prize.x, p2 = prize.y, ky1 = -x2 * y1, ky2 = x1 * y2,
            kp1 = -x2 * p1, kp2 = x1 * p2, sumY = ky1 + ky2, sumP = kp1 + kp2;
    if (sumP % sumY == 0)
      rB = sumP / sumY;
    else
      continue;

...
static point pos;
static map<char, point> dirs = {
    {'^', {-1, 0}}, {'v', {1, 0}}, {'<', {0, -1}}, {'>', {0, 1}}};
static map<char, char> oppDir = {
    {'^', 'v'}, {'v', '^'}, {'<', '>'}, {'>', '<'}};

static void MovBot(char dir, point &pos) {
  point cur = pos + dirs[dir];
  while (grid[cur.y][cur.x] == 'O')
    cur = cur + dirs[dir];
  if (grid[cur.y][cur.x] == '#')
    return;
...
static bool ChkBox(char dir, point pos) {
  ...
  if (grid[cur.y][cur.x] == '#')
    return false;
  if (grid[cur.y][cur.x] == '[')
    return ChkBox(dir, cur) &&
           (dirs[dir].x + 1 == 0 ? true : ChkBox(dir, {cur.y, cur.x + 1}));
  if (grid[cur.y][cur.x] == ']')
    return ChkBox(dir, cur) &&
           (dirs[dir].x - 1 == 0 ? true : ChkBox(dir, {cur.y, cur.x - 1}));
  return false;
